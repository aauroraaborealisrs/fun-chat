...
function validateInput(
  ...
  if (!regex.test(inputElement.value)) {
    ...
    if (inputId === "login") {
      errorElement.textContent =
        "Логин должен состоять только из латинских букв и быть не менее 3 символов";
    } else if (inputId === "password") {
      if (inputElement.value && inputElement.value.trim() !== "") {
        isValid = false;
        errorElement.textContent =
          "Пароль должен содержать минимум 1 латинскую букву и 1 цифру.";
      }
    } else {
      errorElement.textContent =
...
function checkInputs() {
  ...
  const loginValid = validateInput("login", "loginError", /^[A-Za-z]{3,}$/, 3);
